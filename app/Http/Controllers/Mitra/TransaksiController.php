...
class TransaksiController extends Controller
{
	public function index(){
		...
		if ($produk == 0){
			...
			$total_pemasukan = DB::table('transaksi')->select(DB::raw('sum(harga_total) as harga_total'))->where('toko_id', $toko->id)->where('jenis', 'pemasukan')->first()->harga_total;
			$total_pengeluaran = DB::table('transaksi')->select(DB::raw('sum(harga_total) as harga_total'))->where('toko_id', $toko->id)->where('jenis', 'pengeluaran')->first()->harga_total;
			// dd($data_pekanan_transaksi);
			// dd($total_pengeluaran);
            // dd($pesanan);
			return view('/users/user/m-mitra/premium/transaksi/index', compact('toko', 'data_pekanan_hari', 'data_pekanan_transaksi', 'data_bulanan_bulan', 'data_bulanan_transaksi','pesanan', 'total_pesanan', 'total_pemasukan', 'total_pengeluaran'));
		}
	}
	...
	public function pilih_rentan_tanggal(Request $request){
		...
		if ($request->rentan == 'Semua'){
			...
			$total_pemasukan = DB::table('transaksi')->select(DB::raw('sum(harga_total) as harga_total'))->where('toko_id', $toko->id)->where('jenis', 'pemasukan')->where('tanggal', $tanggal)->first()->harga_total;
			$total_pengeluaran = DB::table('transaksi')->select(DB::raw('sum(harga_total) as harga_total'))->where('toko_id', $toko->id)->where('jenis', 'pengeluaran')->where('tanggal', $tanggal)->first()->harga_total;
		}
		...
		$data = array("hasil"=>$result, "pemasukan"=>"IDR. ".number_format($total_pemasukan,0,',','.'), "pengeluaran"=>"IDR. ".number_format($total_pengeluaran,0,',','.'), "keuntungan"=>"IDR. ".number_format($total_pemasukan-$total_pengeluaran,0,',','.'));
		echo json_encode($data, JSON_FORCE_OBJECT);
		// echo $request->rentan;
	}

	public function pilih_rentan_tanggal_range(Request $request){
		$toko = Toko::where('users_id', Auth()->user()->id)->first();
		date_default_timezone_set('Asia/Makassar');

		$tanggal_terakhir = $request->tanggal_akhir;
	...
	public function laporan_keuangan(){
		...
		$transaksi = Transaksi::where('toko_id', $toko->id)->where('tanggal', $tanggal)->orderBy('tanggal', 'asc')->get();	
		$total_pemasukan = DB::table('transaksi')->select(DB::raw('sum(harga_total) as harga_total'))->where('toko_id', $toko->id)->where('jenis', 'pemasukan')->where('tanggal', $tanggal)->first()->harga_total;
		$total_pengeluaran = DB::table('transaksi')->select(DB::raw('sum(harga_total) as harga_total'))->where('toko_id', $toko->id)->where('tanggal', $tanggal)->where('jenis', 'pengeluaran')->first()->harga_total;

		return view('/users/user/m-mitra/premium/transaksi/laporan_keuangan', compact('transaksi', 'total_pemasukan', 'total_pengeluaran'));
	}
