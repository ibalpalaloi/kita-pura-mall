...
<!-- <form enctype="multipart/form-data" action="{{url()->current()}}/simpan" method="post" > -->
    ...
    <main id="homepage" class="homepage" style='background: transparent; padding: 5em 0px 0px 0px;'>
        ...
        <div>
            <div style="padding: 0px 16px 1em;">
                <h3 style="color: white;">Atur Landing Page (ADMIN)</h3>
                ...
                <form id="upload_cover" enctype="multipart/form-data" method="post">
                    ...
                    <div class="input-group mb-3 div-input-mall-square" id="div_video" style="margin-top: 1em; background:transparent; border: none; border-radius: 1.2em;">
                        ...
                        <input type="text" name="id_toko" hidden value="{{$toko->id}}">
    ...
    <script type="text/javascript">
        ...
        function simpan_video(){
            ...
            $.ajax({
                url: "<?=url('/')?>/admin/manajemen/toko/"+toko_id+"/landing_page_ganti_video",
                method: "post",
                data : {link:link, id_toko:toko_id, no:nomor_div, _token:'{{csrf_token()}}'},
                success:function(result)
                {
...
$('.upload-image-maps-1').on('click', function (ev) {
    ...
    resize_maps_1.croppie('result', {
        circle: false,
        type: 'canvas',
        size: foto_1_imageSize2,
        quality: 1
    }).then(function (img) {
        $.ajax({
            url: "<?=url('/')?>/admin/manajemen/toko/"+toko_id+"/landing_page_ganti_foto_maps",
            type: "POST",
            data: {"image":img, "jenis":"1", "size":"320x216", "nama":rString},
            success: function (data) {
            }
        });
    });
...
$('.upload-image-maps-2').on('click', function (ev) {
    ...
    resize_maps_2.croppie('result', {
        circle: false,
        type: 'canvas',
        size: foto_2_imageSize2,
        quality: 1
    }).then(function (img) {
        $.ajax({
            url: "<?=url('/')?>/admin/manajemen/toko/"+toko_id+"/landing_page_ganti_foto_maps",
            type: "POST",
            data: {"image":img, "jenis":"2", "size":"202x320", "nama":rString},
            success: function (data) {
            }
        });
    });
...
$('.upload-image-maps-3').on('click', function (ev) {
    ...
    resize_maps_3.croppie('result', {
        circle: false,
        type: 'canvas',
        size: foto_3_imageSize2,
        quality: 1
    }).then(function (img) {
        $.ajax({
            url: "<?=url('/')?>/admin/manajemen/toko/"+toko_id+"/landing_page_ganti_foto_maps",
            type: "POST",
            data: {"image":img, "jenis":"3", "size":"272x320", "nama":rString},
            success: function (data) {
            }
        });
    });
